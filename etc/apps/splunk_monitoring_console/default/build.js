...
function cleanGeneratedList() {
    var BLOCK_LIST = ['src', 'build_tools', 
            'package.json', 'package-lock.json', 'mc_ui_unit_tests.yml', 'node.cmd',
            'profiles'
        ];

    var contents = fs.readFileSync(GENERATED_LIST_FILE, 'utf8');
    var lines = contents.split('\n');
    var filteredLines = lines.filter(function(line) {
        return !BLOCK_LIST.some(function(skip) {
            return new RegExp('splunk_monitoring_console.*' + skip).test(line);
        });
    });

    fs.writeFileSync(GENERATED_LIST_FILE, filteredLines.join('\n'), 'utf8');
}
