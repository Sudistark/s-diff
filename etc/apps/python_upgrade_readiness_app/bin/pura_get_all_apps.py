...
def get_apps():
    ...
    try:
        ...
        try:
            ...
            http = httplib2.Http()
            head_response = http.request(BASE_URL, "HEAD")
            logging.info("HEAD request response: {}".format(head_response[0].status))
        except Exception as e:
            logging.exception("Exception while initial call to splunkbase API: {}".format(str(e)))
        count = 0
        while count < MAX_COUNT:
            ...
            try:
                ...
                logging.exception("Exception occured while calling splunkbase API: {}".format(str(e)))
                try:
                    http = httplib2.Http()
                    head_response = http.request(BASE_URL, "HEAD")
                    logging.info("HEAD request response: {}".format(head_response[0].status))
                except Exception as e:
                    logging.exception("Exception occured while sending OPTIONS request to splunkbase API: {}".format(str(e)))
            finally:
                count += 1
        if count == MAX_COUNT and response_from_api == False:
            logging.info("Could not get response from splunbase API after {} attempts.".format(count))
            return False
        apps = response.json()
