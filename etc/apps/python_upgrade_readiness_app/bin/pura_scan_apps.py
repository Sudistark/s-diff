...
logging = logger_manager.setup_logging('pura_scan_apps')
...
def get_all_apps(session_key):
    ...
    endpoint = "/services/pura_app_list?type={}".format(APP_LIST_TYPE)
    try:
...
def trigger_scan(apps, session_key):
    ...
    endpoint = "/services/pura_scan_deployment"
...
def poll_scan(session_key):
    ...
    endpoint = "/services/pura_read_progress"
...
if __name__ == "__main__":
    try:
        ...
        try:
            logging.info("Updating Csv before the Scan.")
            pura_get_all_apps.main()
        except Exception as e:
            logging.exception("Updating Csv Error: {}".format(e))
        logging.info("Time taken: {}".format(datetime.now() - start_time))
