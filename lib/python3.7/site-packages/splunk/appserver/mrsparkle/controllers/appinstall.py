...
from splunk.appinspect.client import AppInspectClient
...
from splunk.appserver.mrsparkle.lib.util import allowInstallFromFile
from splunk.appserver.mrsparkle.lib.util import requireAppInspect
...
APP_READ_BUFFER_SIZE = 65535
APPINSPECT_INCLUDED_TAGS = ["private_app"]
APPINSPECT_EXCLUDED_TAGS = []
...
class AppInstallController(BaseController, Capabilities):
    ...
    def getAppIdFromPackage(self, package_path):
        """
        Get the app id from the package, which is the root directory name,
        AppInspect has check to verify the app id matches the name of folder by
        check_that_directory_name_matches_package_id

        :type package_path: string, required
        :param package_path: original package path

        :return app_id: string, package app id
        """
        logger.info(f"Fetching app ID from app package with package_path: {package_path}")
        app_id = None
        with tarfile.open(package_path) as tar:
            for file_info in tar.getmembers():
                file_path = file_info.path.split("/")
                if file_path[0] == ".":
                    # a path can start with "current folder" (.) - skip it
                    file_path = file_path[1:]

                assert len(file_path) > 0

                folder_name = file_path[0]
                # check whether the folder isn't some metadata (starts with dot)
                if not folder_name.startswith("."):
                    app_id = folder_name
                    break
        logger.info(f"app_id of package_path: {package_path} is: {app_id}")
        return app_id

    def generate_SHA(self, filepath):
        """
        Generates SHA of a given file. SHA variant: 256
        Following the same guidance from AppInspect SHA generate logic:
        https://cd.splunkdev.com/apphealth/appinspect/-/blob/master/app/api/request_hasher.py
        ...
        with open(filepath, "rb") as file_obj:
            content = file_obj.read(APP_READ_BUFFER_SIZE)
            while content:
                file_hash.update(content)
                content = file_obj.read(APP_READ_BUFFER_SIZE)

        included_tags = ','.join(APPINSPECT_INCLUDED_TAGS)
        excluded_tags = ','.join(APPINSPECT_EXCLUDED_TAGS)
        tags_str = f"included:{included_tags}-excluded:{excluded_tags}".encode(
            "utf-8"
        )
        file_hash.update(tags_str)
        hash_value = file_hash.hexdigest()
        logger.info(f'SHA256 of file {filepath} is: {hash_value}')
        return hash_value
    ...
    def processAppUpload(self, f, force, **kwargs):
        ...
        try:
            if requireAppInspect():
                package_id = self.getAppIdFromPackage(tmpPath)
                sha = self.generate_SHA(tmpPath)
                appinspect_client = AppInspectClient(
                    kwargs.get("sbuser"),
                    kwargs.get("sbpass")
                )
                appinspect_client.get_validation_result(sha)
                if not force:
                    app_existed = self.getLocalApp(package_id)
                    if app_existed:
                        raise SBFileUploadException(
                            _("App with this name already exists."
                            " Select upgrade app to overwrite the existing app.")
                        )
            ...
            if response.status in (200, 201):
                ...
                if not sbAppId and requireAppInspect():
                    sbAppId = package_id
                return sbAppId
    ...
    def upload(self, appid, return_to=None, breadcrumbs=None, state=None, appfile=None, force=None, **kw):
        ...
        # Do not show in single instance without feature flag enabled (404)
        if not allowInstallFromFile():
