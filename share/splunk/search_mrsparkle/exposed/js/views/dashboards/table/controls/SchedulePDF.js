    ...
    function (module,
        ...
        return Modal.extend({
            ...
            initialize: function () {
                ...
                // Default value comes from a different model, as per SPL-219291, we have to only set the inmem value if needed.
                var hasPaperSize = this.model.inmem.get('action.email.papersize');
                var hasPaperOrientation = this.model.inmem.get('action.email.paperorientation');
                if (!hasPaperSize || !hasPaperOrientation) {
                    pdfUtils.getEmailAlertSettings().done(function (emailSettings) {
                        var paperSize = emailSettings.entry.content.get('reportPaperSize');
                        if (!hasPaperSize && paperSize) {
                        ...
                        if (!hasPaperOrientation && paperOrientation) {
